...
} = require("./broadcasts/broadcastNewSensorData");
const {
  broadcastSensorConnection,
} = require("./broadcasts/broadcastSensorConnection");
    ...
    ws.on("message", async (message) => {
      try {
        ...
        if (!activeSensors.has(credentials.id)) {
          ...
          broadcastSensorConnection(credentials.id, "con", logger, activeUsers, activeVisitors);
        } else {
...
async function handleSensorDisconnection(
  ws,
  activeSensors,
  db,
  logger,
  ...
  const idsToRemove = [...activeSensors.keys()].filter(
    (id) => activeSensors.get(id).ws === ws
  );
  ...
  idsToRemove.forEach(
    async (id) =>
      await db
        .collection("sensors")
        .updateOne({ id: id }, { $set: { active: Date.now() } })
  );

  idsToRemove.forEach((id) =>
    broadcastSensorConnection(id, "discon", logger, activeUsers, activeVisitors)
  );
